...
;(function anonymous(window) {
  ...
  let init = function init(options = {}) {
    let { url, method = 'GET', data = null, dataType = 'JSON', async = true, cache = true, success, error } = options
    // =>MOUNT:把配置项挂载到实例上
...
;(function myAxios(window) {
  ...
  let ajaxPromise = function ajaxPromise(options) {
    ...
    let { method, url, baseUrl, data, dataType, headers, params, cache } = options
    // =>把传递的参数进一步处理
